...
sql = 'SELECT data FROM execution_data WHERE "executionId" = 132;'
...
def resolve(v):
    if isinstance(v, str) and v.isdigit():
        ...
        if idx < len(data):
            ...
            # We don't remove from visited to prevent re-parsing same index
            # but we update with the actual result for memoization
            visited[idx] = res
            return res
...
with open('exec_132_resolved.json', 'w') as f:
