...
async function run() {
    ...
    try {
        const id = 'lW5mGPOkiPI7jHXs'; // V2 Workflow
        const res = await client.query('SELECT nodes FROM workflow_entity WHERE id = $1', [id]);
        ...
        // 1. Update SEO Generation Node
        const seoNode = nodes.find(n => n.name === 'SEO Generation');
        if (seoNode) {
            console.log('Updating SEO Generation node...');
        ...
        // 2. Update Persona Classification Node
        const personaNode = nodes.find(n => n.name === 'Persona Classification');
        if (personaNode) {
            console.log('Updating Persona Classification node...');
        ...
        // 4. Update DB
        await client.query('UPDATE workflow_entity SET nodes = $1, settings = $2, "updatedAt" = NOW(), "versionCounter" = "versionCounter" + 1 WHERE id = $3', [JSON.stringify(nodes), JSON.stringify(settings), id]);
        console.log('Workflow high-limit optimization applied and logging enabled.');
